...
enum proto_state { wait_CON = 0, CON_sent = 1, CONNECTED = 2, RESENDING = 3 };     // States

// Events
enum proto_event { RCV_CON = 0, RCV_FIN = 1, RCV_ACK = 2, RCV_DATA = 3,
                   CONNECT = 4, CLOSE = 5,   SEND = 6,    TIMEOUT = 7, TIMEOUT_RESEND = 8 };

char *pkt_name[] = { "F_CON", "F_FIN", "F_ACK", "F_DATA" };
char *st_name[] =  { "wait_CON", "CON_sent", "CONNECTED", "RESENDING" };
char *ev_name[] =  { "RCV_CON", "RCV_FIN", "RCV_ACK", "RCV_DATA",
                     "CONNECT", "CLOSE",   "SEND",    "TIMEOUT", "TIMEOUT_RESEND" };
...
{   
    ...
    printf("Resend Data to peer '%s' size:%d try:%d/%d\n",
        recent_sent_data, recent_sent_data_size, ++resending_try, TIMEOUT_RESEND);
    send_packet(F_DATA, (struct p_event *)p, ((struct p_event *)p)->size);

    sprintf(((struct p_event*)p)->packet.data, "%s", recent_sent_data);
    ((struct p_event*)p)->size = recent_sent_data_size;

    send_packet(F_DATA, (struct p_event *)p, ((struct p_event *)p)->size);
    set_timer(RESENDING_TIMEOUT);
}
struct state_action p_FSM[NUM_STATE][NUM_EVENT] = {
  ...
  {{ passive_con, CONNECTED }, { close_con, wait_CON }, { report_connect, CONNECTED }, { NULL,      CON_sent },
   { NULL,        CON_sent },  { close_con, wait_CON }, { NULL,           CON_sent },  { close_con, wait_CON },     { NULL, CON_sent}},

  // - CONNECTED state
  {{ NULL, CONNECTED },        { close_con, wait_CON }, { NULL,      CONNECTED },      { report_data, CONNECTED },
   { NULL, CONNECTED },        { close_con, wait_CON }, { send_data, CONNECTED },      { resend_data, RESENDING },  { NULL, CONNECTED}},

   // - RESENDING state
   {{ NULL, RESENDING},         { close_con, wait_CON},   { NULL,        CONNECTED},      { report_data, RESENDING },
    { NULL, RESENDING},         { close_con, wait_CON},   { NULL,        CONNECTED},      { resend_data, RESENDING },  { stop_resending, wait_CON}}
};
...
loop:
    ...
    if (!kbhit()) {
        ...
        if(timedout) {
            if(resending_try < RESENDING_TIMEOUT){
                event.event = TIMEOUT;
            }else{
                resending_try = 0;
                event.event = TIMEOUT_RESEND;
            }
        } else {
